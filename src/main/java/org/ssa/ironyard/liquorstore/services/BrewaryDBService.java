            ...
            try
            {
                ...
                for (int i = 0; i < array.size(); i++)
                {
                    ...
                    String is_final_page = page.get("is_final_page").asText();
                     ...
                     jb = new JsonBeer(name,tags,regular_price_in_cents,primary_category,secondary_category,origin,packageP,package_unit_volume_in_milliliters,
...
//           }
           ...
           coreProduct = new CoreProduct.Builder().name(name).tags(tags).type(type).subType(subType).description(description).fullSizeImage(fullUrl).thumbnail(thumbUrl).loaded(false).build();
